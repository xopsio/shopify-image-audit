...
def _sample_comparison_result(
    *,
    lcp_before: float = 4200.0,
    lcp_after: float = 1800.0,
    ...
    return ComparisonResult(
        ...
        vitals=VitalsDelta(
            ...
            cls=MetricDelta(before=0.18, after=0.05, delta=-0.13, delta_pct=-72.2, status="improved"),
            inp=MetricDelta(before=320.0, after=120.0, delta=-200.0, delta_pct=-62.5, status="improved"),
            ttfb=MetricDelta(before=900.0, after=400.0, delta=-500.0, delta_pct=-55.6, status="improved"),
        ),
        images=ImageStatsDelta(
            before_count=2,
            after_count=2,
            count_delta=0,
            before_total_bytes=165_000,
            after_total_bytes=165_000,
            total_bytes_delta=0,
            before_total_waste=10_000,
            after_total_waste=10_000,
            total_waste_delta=0,
            before_avg_score=87.5,
            after_avg_score=87.5,
            avg_score_delta=0.0,
        ),
...
class TestPrintComparisonTable:
    ...
    def test_regressed_renders_red_markup(self) -> None:
        comp = _sample_comparison_result(lcp_before=1800.0, lcp_after=4200.0, lcp_status="regressed")
        console, buf = _capture_console()
        print_comparison_table(comp, console=console)
        out = buf.getvalue()
        assert "[red]" in out
    ...
    def test_unchanged_renders_dim(self) -> None:
        comp = _sample_comparison_result(lcp_before=2000.0, lcp_after=2000.0, lcp_status="unchanged")
        # Force delta to zero
        comp = comp.model_copy(
            update={
                "vitals": comp.vitals.model_copy(
                    update={
                        "lcp": MetricDelta(before=2000.0, after=2000.0, delta=0, delta_pct=0.0, status="unchanged"),
                    }
                ),
            }
        )
...
class TestPrintComparisonSummary:
    ...
    def test_regressions_section_rendered(self) -> None:
        ...
        comp = comp.model_copy(
            update={
                "summary": comp.summary.model_copy(
                    update={
        ...
        with patch("rich.print", side_effect=lambda *a, **kw: captured.write(" ".join(str(x) for x in a) + "\n")):
            print_comparison_summary(comp)
