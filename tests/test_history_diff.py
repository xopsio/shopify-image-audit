    ...
    return AuditResult.model_validate({
        ...
        "images": [
            {"src": "https://cdn.example.com/hero.jpg", "role": "hero",
             "score": 50, "bytes": 1_200_000, "mime": "image/jpeg"},
        ],
...
def baseline_after() -> AuditResult:
    return AuditResult.model_validate({
        ...
        "images": [
            {"src": "https://cdn.example.com/hero.webp", "role": "hero",
             "score": 85, "bytes": 95_000, "mime": "image/webp"},
        ],
        "summary": {"top_issues": []},
    })
...
@pytest.fixture
def populated_history(tmp_path: Path, baseline_before: AuditResult,
                     baseline_after: AuditResult) -> HistoryStore:
...
class TestCompareEntries:
    def test_returns_comparison_result(
        self, populated_history: HistoryStore, baseline_before: AuditResult,
    ...
    def test_returns_none_for_unknown_id(self, populated_history: HistoryStore) -> None:
        assert populated_history.compare_entries(
            "mystore.myshopify.com", "deadbeef0000", "deadbeef0001",
        ) is None

    def test_returns_none_when_only_one_known(
        self, populated_history: HistoryStore, baseline_after: AuditResult,
    ) -> None:
        entries = populated_history.list_entries("mystore.myshopify.com")
        valid_id = entries[0].id
        # One valid, one unknown
        assert populated_history.compare_entries(
            "mystore.myshopify.com", valid_id, "deadbeef9999",
        ) is None
...
class TestGenerateDiffHtml:
    def test_renders_vital_deltas(
        ...
        comparison = populated_history.compare_entries(
            "mystore.myshopify.com", entries[1].id, entries[0].id,
        )
        html = generate_diff_html(
            "mystore.myshopify.com", entries[1], entries[0], comparison,
        )
    ...
    def test_renders_roi_estimate(
        self, populated_history: HistoryStore,
    ) -> None:
        entries = populated_history.list_entries("mystore.myshopify.com")
        comparison = populated_history.compare_entries(
            "mystore.myshopify.com", entries[1].id, entries[0].id,
    ...
    def test_accepts_dict_form(
        ...
        html = generate_diff_html(
            "mystore.myshopify.com", entries[1], entries[0], comparison_dict,
        )
        assert "Audit Diff" in html
...
class TestHistoryDiffCli:
    ...
    def test_diff_unknown_id_exits_2(
        ...
        result = runner.invoke(app, [
            "history", "diff", "mystore.myshopify.com",
            "--from", "deadbeef0000", "--to", "deadbeef0001",
            "--history-dir", str(tmp_path),
        ])
