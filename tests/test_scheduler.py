...
class TestScheduleConfig:
    def test_from_dict_minimal(self) -> None:
        cfg = ScheduleConfig.from_dict({
            "shop_domain": "a.myshopify.com", "url": "https://a.myshopify.com",
        })
...
class TestRunAllSchedules:
    ...
    def test_success_records_to_history(
        self, tmp_path: Path, sample_audit_result: AuditResult,
    ) -> None:
        schedule_store = ScheduleStore(tmp_path / "schedules")
        schedule_store.add(ScheduleConfig(
            "mystore.myshopify.com", "https://mystore.myshopify.com",
        ))
        history_store = HistoryStore(base_dir=tmp_path / "history")

        with patch("engine.cli_helpers._dispatchers.fetch_url_as_audit",
                   return_value=sample_audit_result):
            results = run_all_schedules(schedule_store, history_store=history_store)
    ...
    def test_history_record_failure_surfaces_error(
        ...
        with patch("engine.cli_helpers._dispatchers.fetch_url_as_audit",
            ...
            results = run_all_schedules(
                schedule_store, history_store=FailingHistoryStore(),
            )
...
class TestScheduleCli:
    ...
    def test_add_then_list(self, tmp_path: Path) -> None:
        result = runner.invoke(app, [
            "schedule", "add", "mystore.myshopify.com", "https://mystore.myshopify.com",
            "--label", "Daily",
            "--schedule-dir", str(tmp_path),
        ])
        assert result.exit_code == 0
    ...
    def test_remove(self, tmp_path: Path) -> None:
        runner.invoke(app, [
            "schedule", "add", "a.myshopify.com", "https://a",
            "--schedule-dir", str(tmp_path),
        ])
        result = runner.invoke(app, [
            "schedule", "remove", "a.myshopify.com",
            "--schedule-dir", str(tmp_path),
        ])
        assert result.exit_code == 0
