...
Exit codes (spec):
    0  – success
    2  – invalid arguments / invalid JSON / validation errors
    10 – lighthouse execution failure
"""
...
def run(
    ...
    # Invalid JSON or schema-validation failures → exit 2
    # All other pipeline errors → exit 10 (unexpected / lighthouse-side failure)
    try:
        result: AuditResult = run_audit(json_path, url=url, device=device, runs=runs)
    except (json.JSONDecodeError, ValueError, TypeError) as exc:
        rprint(f"[red]Error:[/red] Invalid input or validation failure: {exc}")
        raise typer.Exit(code=EXIT_INVALID_ARGS)
...
def extract(
    ...
    if not isinstance(raw, dict):
        rprint("[red]Error:[/red] Unexpected JSON type: expected an object.")
        raise typer.Exit(code=EXIT_INVALID_ARGS)

    try:
        images = parse(raw)
    except (TypeError, ValueError) as exc:
        rprint(f"[red]Error:[/red] Failed to parse Lighthouse JSON: {exc}")
        raise typer.Exit(code=EXIT_INVALID_ARGS)
...
def score(
    ...
    if not isinstance(images, list):
        rprint("[red]Error:[/red] Unexpected JSON type: expected an array.")
        raise typer.Exit(code=EXIT_INVALID_ARGS)

    try:
        scored = rank(images)
    except (TypeError, ValueError) as exc:
        rprint(f"[red]Error:[/red] Failed to score images: {exc}")
        raise typer.Exit(code=EXIT_INVALID_ARGS)

    typer.echo(json.dumps(scored, indent=2))
    raise typer.Exit(code=EXIT_OK)
...
def report(
    ...
    try:
        with open(audit_result_json, encoding="utf-8") as f:
            data = json.load(f)
    except json.JSONDecodeError as exc:
        rprint(f"[red]Error:[/red] Invalid JSON: {exc}")
        raise typer.Exit(code=EXIT_INVALID_ARGS)
    ...
    url = html.escape(str(meta.get("url", "")))
    device = html.escape(str(meta.get("device", "")))
    ...
    for img in images:
        ...
        rows += (
            "<tr>"
            f"<td>{html.escape(str(img.get('src', '')))}</td>"
            f"<td>{html.escape(str(img.get('role', '')))}</td>"
            f"<td>{html.escape(str(img.get('score', '')))}</td>"
            f"<td>{html.escape(str(img.get('bytes', '')))}</td>"
            f"<td>{'Y' if img.get('is_lcp_candidate') else ''}</td>"
            f"<td>{html.escape(str(img.get('recommendation', '') or ''))}</td>"
            "</tr>\n"
        )

    issues_html = "".join(f"<li>{html.escape(str(issue))}</li>" for issue in top_issues)

    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Shopify Image Audit – {url}</title>
  <style>
...
<body>
  <h1>Shopify Image Audit</h1>
  <p><strong>URL:</strong> {url}</p>
  <p><strong>Device:</strong> {device} &nbsp; <strong>Generated:</strong> {timestamp}</p>
  <h2>Images</h2>
  <table>
    <thead>
      <tr><th>src</th><th>role</th><th>score</th><th>bytes</th><th>LCP?</th><th>recommendation</th></tr>
    </thead>
    <tbody>
{rows}    </tbody>
  </table>
